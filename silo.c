...
void editorDrawRows(struct abuf *ab) {
    ...
    if (y == E.screenrows / 3) {
      ...
      int welcomelen = snprintf(welcome, sizeof(welcome),
        "Silo editor 🔫  version %s", SILO_VERSION);
...
//On exit restore flags
void disableRawMode() {
	if(tcsetattr(STDIN_FILENO, TCSAFLUSH, &E.orig_termios)==-1) die("tcsetattr");
}

//Flags for input normalisation
void enableRawMode() {
...
int editorReadKey() {
	//wait for 1 keypress and return it
	int nread;
	char c;
...
void editorProcessKeyPress() {
	...
	//TODO: control keys and editor functions
